 ...
 */
package cc.dobot.crtcpdemo.widget;
...
public class HVScrollView extends ViewGroup implements NestedScrollingParent, NestedScrollingChild, ScrollingView {
    ...
    private long childTouchDownTimes=0L;
...
//    private boolean mIsBeingDragged;
    ...
    public void setOnChildTouchListener(OnChildTouchListener listener) {
        this.mOnChildTouchListener=listener;
    }

    public interface OnChildTouchListener{
        void onChildTouch(View childView);
    }
    /**
...
//    private boolean mIsBeingDragged = false;
    ...
    private boolean inChild(int x, int y) {
        if (getChildCount() > 0) {
            ...
            boolean res= !(y < child.getTop() - scrollY
            ...
            {
                childTouch=res;
                childTouchDownTimes=System.currentTimeMillis();
            }
            return res;
...
//        dispatchOnScrollStateChanged(state);
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        boolean needInterceptByScroll=false;
        boolean isHorizontallyScroll=false;
...
//                        mIsBeingDragged = true;
            ...
            case MotionEvent.ACTION_DOWN: {
                ...
                if (y>=getBottom()-75){
                    needInterceptByScroll=true;
                    isHorizontallyScroll=true;
                }else
                if (!inChild(x, y)) {
                    setScrollState(SCROLL_STATE_IDLE);
                    recycleVelocityTracker();
...
//                }
                ...
                if (childTouch)
                {
                    long downTime=System.currentTimeMillis()-childTouchDownTimes;
                    if (downTime<=500&&mOnChildTouchListener!=null)
                        mOnChildTouchListener.onChildTouch(null);
                    childTouchDownTimes=0L;
                    childTouch=false;
                }
                break;
...
//                        mVelocityTracker.clear();
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int measuredWidth = View.resolveSizeAndState(maxWidth, widthMeasureSpec, childState);
        int measuredHeight = View.resolveSizeAndState(maxHeight, heightMeasureSpec, childState << MEASURED_HEIGHT_STATE_SHIFT);
        setMeasuredDimension(measuredWidth, measuredHeight);
        //System.out.println("on HVScrollView Measure 8");
/*        boolean needMeasure = false;
        ...
        if (needMeasure) {
            System.out.println("child remeasure");
            child.measure(childWidthMeasureSpec, childHeightMeasureSpec);
        }*/
    }
