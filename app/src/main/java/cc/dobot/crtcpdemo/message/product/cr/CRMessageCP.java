...
public class CRMessageCP extends BaseMessage {

    private int R=1;
    @Override
    public void constructSendData() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            this.messageContent=("CP("+R+")").getBytes(StandardCharsets.US_ASCII);
        }else
            this.messageContent=("CP("+R+")").getBytes( Charset.forName("US-ASCII"));
        this.messageStrContent=("CP("+R+")");
    }
