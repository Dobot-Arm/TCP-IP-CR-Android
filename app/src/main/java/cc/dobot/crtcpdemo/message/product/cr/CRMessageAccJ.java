...
public class CRMessageAccJ extends BaseMessage {

    private int R=1;
    @Override
    public void constructSendData() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            this.messageContent=("AccJ("+R+")").getBytes(StandardCharsets.US_ASCII);
        }else
            this.messageContent=("AccJ("+R+")").getBytes( Charset.forName("US-ASCII"));
        this.messageStrContent=("AccJ("+R+")");
    }
