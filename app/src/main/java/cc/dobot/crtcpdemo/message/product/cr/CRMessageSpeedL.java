...
public class CRMessageSpeedL extends BaseMessage {

    private int R=1;
    @Override
    public void constructSendData() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            this.messageContent=("SpeedL("+R+")").getBytes(StandardCharsets.US_ASCII);
        }else
            this.messageContent=("SpeedL("+R+")").getBytes( Charset.forName("US-ASCII"));
        this.messageStrContent=("SpeedL("+R+")");
    }
