...
public class CRMessageSetObstacleAvoid extends BaseMessage {

    private int status=1;
    @Override
    public void constructSendData() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            this.messageContent=("SetObstacleAvoid("+status+")").getBytes(StandardCharsets.US_ASCII);
        }else
            this.messageContent=("SetObstacleAvoid("+status+")").getBytes( Charset.forName("US-ASCII"));
        this.messageStrContent=("SetObstacleAvoid("+status+")");
    }
