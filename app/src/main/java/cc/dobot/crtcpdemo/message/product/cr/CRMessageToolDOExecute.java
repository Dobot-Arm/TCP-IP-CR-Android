...
public class CRMessageToolDOExecute extends BaseMessage {
    ...
    public void constructSendData() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            this.messageContent=("ToolDOExecute("+index+","+status+")").getBytes(StandardCharsets.US_ASCII);
        }else
            this.messageContent=("ToolDOExecute("+index+","+status+")").getBytes( Charset.forName("US-ASCII"));
        this.messageStrContent=("ToolDOExecute("+index+","+status+")");
    }
