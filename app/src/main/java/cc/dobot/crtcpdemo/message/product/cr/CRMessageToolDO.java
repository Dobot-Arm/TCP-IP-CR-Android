...
public class CRMessageToolDO extends BaseMessage {
    ...
    public void constructSendData() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            this.messageContent=("ToolDO("+index+","+status+")").getBytes(StandardCharsets.US_ASCII);
        }else
            this.messageContent=("ToolDO("+index+","+status+")").getBytes( Charset.forName("US-ASCII"));
        this.messageStrContent=("ToolDO("+index+","+status+")");
    }
