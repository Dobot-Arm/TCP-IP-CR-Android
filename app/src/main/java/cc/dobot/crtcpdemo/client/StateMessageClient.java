...
public class StateMessageClient extends TcpClient {

    private static final String TAG = StateMessageClient.class.getSimpleName();

    private volatile int seqNum = 1; //递增自然数

    private static StateMessageClient instance;
    ...
    private StateRefreshListener listener;

    private StateMessageClient() {
    ...
    public void setListener(StateRefreshListener listener) {
        this.listener = listener;
    }

    public static StateMessageClient getInstance() {
        if (instance == null) {
            synchronized (StateMessageClient.class) {
                if (instance == null) {
                    instance = new StateMessageClient();
                }
            }
        }
        return instance;
